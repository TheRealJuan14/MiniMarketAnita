...
const createProveedor = async (req, res, next) => {
    try {
        ...
        const { rows } = await pool.query(
            'INSERT INTO proveedores (nombre, ruc, direccion, telefono, email) VALUES ($1, $2, $3, $4, $5) RETURNING *',
...
const updateProveedor = async (req, res, next) => {
    try {
        ...
        const { nombre, ruc, direccion, telefono, email } = req.body;
        const { rows } = await pool.query(
            'UPDATE proveedores SET nombre = $1, ruc = $2, direccion = $3, telefono = $4, email = $5 WHERE id = $6 RETURNING *',
